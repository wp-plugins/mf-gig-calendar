...
class GigCalendarWidget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( $widget_date_format == "" ) {
			$widget_date_format = 'D j M';
		}
	...
	function form( $instance ) {
		...
		if ( $link_text == "" ) {
			$link_text = __( 'View My Event Calendar', 'mfgigcal' );
		}
		if ( $widget_date_format == "" ) {
			$widget_date_format = 'D j M';
		}
        ...
        <p>
            <label><?php _e( 'How many events to display?', 'mfgigcal' ); ?>
                <input style="width:30px;" id="<?php echo $this->get_field_id( 'events_to_display' ); ?>"
                       name="<?php echo $this->get_field_name( 'events_to_display' ); ?>" type="text"
                       value="<?php echo $events_to_display; ?>"/>
            </label>
        </p>
        <p>
            <label><input class="link_detail_switch" id="<?php echo $this->get_field_id( 'display_when_empty' ); ?>"
                          name="<?php echo $this->get_field_name( 'display_when_empty' ); ?>" type="checkbox"
                          value="1" <?php checked( '1', $display_when_empty ); ?> />
				<?php _e( 'Display the widget even when there are no upcoming events.', 'mfgigcal' ); ?>
            </label>
        </p>
        <p>
            <label><?php _e( 'Date Format:', 'mfgigcal' ); ?>
                <input style="width:70px;" id="<?php echo $this->get_field_id( 'widget_date_format' ); ?>"
                       name="<?php echo $this->get_field_name( 'widget_date_format' ); ?>" type="text"
                       value="<?php echo $widget_date_format; ?>"/>
            </label> <a href="http://codex.wordpress.org/Formatting_Date_and_Time"
                        title="http://codex.wordpress.org/Formatting_Date_and_Time"
                        target="_blank"><?php _e( 'Help' ); ?>?</a>
        </p>
        <p>
            <label><input class="link_detail_switch" id="<?php echo $this->get_field_id( 'calendar_link' ); ?>"
                          name="<?php echo $this->get_field_name( 'calendar_link' ); ?>" type="checkbox"
                          value="1" <?php checked( '1', $calendar_link ); ?> />
